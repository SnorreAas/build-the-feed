...
export function getUserFollowersIdRef(
  uid: string,
  id: string
): DatabaseReference {
  const root = getDatabaseRoot();
  return child(root, `/users/${uid}/followers/${id}`);
}

export function getUserFollowingIdRef(
  uid: string,
  id: string
): DatabaseReference {
  const root = getDatabaseRoot();
  return child(root, `/users/${uid}/following/${id}`);
}
